...
class SpaceNet7(Dataset):
    ...
    def padding(self, sample):
        ...
        if H != 1024:
            image = np.pad(
                image, (((0, 0), (1, 0), (0, 0))), "constant", constant_values=(0)
            )
            mask = np.pad(
                mask, (((0, 0), (1, 0), (0, 0))), "constant", constant_values=(0)
            )

        if W != 1024:
            image = np.pad(
                image, (((0, 0), (0, 0), (1, 0))), "constant", constant_values=(0)
            )
            mask = np.pad(
                mask, (((0, 0), (0, 0), (1, 0))), "constant", constant_values=(0)
            )
...
class SpaceNet7DataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        train_files, test_files = train_test_split(
            files, test_size=0.1, random_state=self.args.seed
        )
        self.spaceNet7_train = SpaceNet7(
            train_files, self.args.crop_size, self.args.exec_mode
        )
        self.spaceNet7_val = SpaceNet7(
            test_files, self.args.crop_size, self.args.exec_mode
        )

    def train_dataloader(self):
        train_sampler = self.ImageSampler(
            len(self.spaceNet7_train), self.args.samples_per_epoch
        )
        train_bSampler = BatchSampler(
            train_sampler, batch_size=self.args.batch_size, drop_last=True
        )
        return DataLoader(
            self.spaceNet7_train,
